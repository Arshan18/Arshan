      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-2.5 sm:space-y-3 mb-8 sm:mb-12">
          ...
          <p className="text-slate-600 text-xs sm:text-base font-medium">
            Data analytics dashboards, big data PySpark pipelines, and Agentic AI research systems.
          </p>
        </div>
        ...
        {/* Projects Cards Grid with Focus & Spotlight Effect */}
        <div
        ...
        >
          {filteredProjects.map((project) => {
            ...
            const currentMouse = mousePos[project.id] || { x: 50, y: 50 };
            ...
            return (
              <div
                ...
                className={`relative flex flex-col justify-between p-5 sm:p-8 rounded-2xl sm:rounded-3xl bg-white border transition-all duration-300 group text-left overflow-hidden ${
                  isHovered
                    ? 'border-blue-500 shadow-2xl shadow-blue-500/15 scale-[1.02] -translate-y-2 z-20 opacity-100 ring-2 ring-blue-500/20'
                    : isDimmed
                    ? 'border-slate-200/60 opacity-55 scale-[0.98] blur-[0.2px] shadow-none'
                    : 'border-slate-200/90 shadow-sm opacity-100 scale-100 hover:border-blue-400'
                }`}
              >
                ...
                <div className="space-y-4 sm:space-y-5 relative z-10">
                  ...
                  <div className="flex items-start justify-between gap-3 sm:gap-4">
                    <div className="space-y-2">
                      {project.badge && (
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full text-[11px] sm:text-xs font-bold transition-colors ${
                          isHovered ? 'bg-blue-600 text-white shadow-sm' : 'bg-blue-600 text-white shadow-2xs'
                        }`}>
                          <span>{project.badge}</span>
                        </span>
                      )}
                      <h3 className={`text-lg sm:text-xl font-extrabold transition-colors leading-snug ${
                        isHovered ? 'text-blue-600' : 'text-slate-900 group-hover:text-blue-600'
                      }`}>
                        {project.title}
                      </h3>
                      {project.role && (
                ...
                <div className="pt-4 sm:pt-6 mt-4 sm:mt-6 border-t border-slate-200 flex flex-wrap items-center justify-between gap-2.5 sm:gap-3 relative z-10">
                  <div className="flex flex-wrap gap-1.5 sm:gap-2">
                    {project.tags.map((tag, idx) => (
                      <span
                        key={idx}
                        className={`px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-md text-[11px] sm:text-xs font-mono font-semibold transition-colors ${
                          isHovered
